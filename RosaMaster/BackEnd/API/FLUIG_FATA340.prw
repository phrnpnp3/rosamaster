...
WSMETHOD POST BODY64 WSSERVICE F_FATA340
	...
	Begin Sequence
		...
		// Validation
		aRetValid := validJson(oBody, {"ARQUIVO", "DESCRICAO", "FILIAL", "TABELA", "BASE64", "ENTIDADE"})
		If !aRetValid[1]
...
WSMETHOD POST BODYURL WSSERVICE F_FATA340
	...
	Local cPathFluig:= SuperGetMV("P3_PTFLUIG",.F.,"/volume/stream/Rmx1aWc=")
	// Local aHeader	:= {}
	Local cMsgError	:= ""
	Local oError    := ErrorBlock({|e| anyError(e, @cMsgError) })
	...
	Begin Sequence
		...
		// Validation
		aRetValid := validJson(oBody, {"ARQUIVO", "DESCRICAO", "FILIAL", "TABELA", "ENTIDADE", "URLD"})
		If !aRetValid[1]
			SetRestFault(400, aRetValid[2])
			return .F.
		EndIf
		...
		cFileName := U_CARACESP(FwNoAccent(DecodeUTF8(ALLTRIM(oBody['ARQUIVO']), "cp1252")))
        ...
        aRetGFile := smGetFile(cBaseUrlFl+cPathFluig,cFile)

        if aRetGFile[1]
            oBody['BINARY'] := aRetGFile[3]
        else
            cMsgError := aRetGFile[2]
            break
        endif

		oMeta := JsonObject():new()
		oMeta['fileName'] 	:= U_CARACESP(FwNoAccent(DecodeUTF8(ALLTRIM(oBody['ARQUIVO']), "cp1252")))
...
WSMETHOD POST BODYCODE WSSERVICE F_FATA340
	...
	Begin Sequence
		...
		// Validation
		aRetValid := validJson(oBody, {"ARQUIVO", "DESCRICAO", "FILIAL", "TABELA", "ENTIDADE", "DOCUMENTO"})
		If !aRetValid[1]
			SetRestFault(400, aRetValid[2])
			return .F.
		EndIf
...
WSMETHOD POST SEARCH WSSERVICE F_FATA340
	...
	Begin Sequence
		...
		// Validation
		aRetValid := validJson(oBody, {"AC9_ENTIDA", "AC9_FILENT", "AC9_CODENT"})
		If !aRetValid[1]
			SetRestFault(400, aRetValid[2])
			return .F.
		EndIf
		...
		While (cAliasAC9)->(!EOF());
			...
			// If (cAliasACB)->(DbSeek(xFilial("ACB")+(cAliasAC9)->AC9_CODOBJ))
			If (cAliasACB)->(DbSeek(iif(xFilial(cAliasACB) == "", (cAliasAC9)->AC9_FILENT, xFilial(cAliasACB)) + (cAliasAC9)->AC9_CODOBJ))
				// aFile 					:= smFileTo64((cAliasACB)->ACB_OBJETO, cAC9FILENT)
				aFile 					:= smFileTo64((cAliasACB)->ACB_OBJETO, iif(xFilial(cAliasACB) == "", (cAliasAC9)->AC9_FILENT, xFilial(cAliasACB)))
...
Static function smSaveFile(oMeta)
	Local aResponse		:= {.F., "", "", ""}
	Local cFileName 	:= cValToChar(Randomize(1,1000))+"_"+DToS(Date())+StrTran(Time(), ":", "")+"_"+U_CARACESP(FwNoAccent(DecodeUTF8(ALLTRIM(oMeta['fileName']), "cp1252")))
	Local cFileIn64 	:= oMeta['fileIn64']
	Local oFileInBin 	:= oMeta['fileInBin']
	Local cDirF			:= AllTrim(cEmpAnt )
	Local cBasePath		:= SuperGetMv("SM_PATHSAV",,"\dirdoc\co"+cDirF+"\")
	// Local cBasePath		:= SuperGetMv("SM_PATHSAV",,"\dirdoc\co99\")
	Local cFileFulNa	:= cBasePath

	// IF Empty(oMeta['branch'])
	IF AllTrim(POSICIONE("SX2",1,"ACB","X2_MODO")) == "C"
		cFileFulNa += "shared\"
	ELSE
		cFileFulNa += "br"+oMeta['branch']+"\"
	ENDIF

	if !ExistDir("co"+cDirF)
        MakeDir("co"+cDirF)
    endif

    if !ExistDir(cFileFulNa)
        MakeDir(cFileFulNa)
    endif
...
Static Function fgravaAC9(oBody)
	...
	Begin Transaction
		...
		ACB->ACB_OBJETO := U_CARACESP(FwNoAccent(DecodeUTF8(ALLTRIM(oBody:ARQUIVO), "cp1252")))
		ACB->ACB_DESCRI := U_CARACESP(FwNoAccent(DecodeUTF8(ALLTRIM(oBody:DESCRICAO), "cp1252")))
		ACB->(MsUnlock())
...
Static Function smGetFile(cHost,cPath)
    ...
    if !EMPTY(cHost)
        ...
        Else
            cMsgError := "Falha ao baixar o arquivo: "+cHost+cPath+", Error:"+oRest:GetLastError()
        EndIf
    else
        cMsgError := "Host não informado."
    endif

    aRet := {lRet,cMsgError,cResult}
    restArea(wAreaATU)
